...
if [ "$GLOBAL" = true ]; then
  ...
  INSTALL_DIR="./.opencode/plugins/opencode-plugin-notify"
...
if [ "$GLOBAL" = true ]; then
  ...
  echo "1. 复制配置文件："
  echo "   mkdir -p ~/.config/opencode"
  echo "   cp .opencode/plugins/opencode-plugin-notify/notify-config.example.json ~/.config/opencode/notify-config.json"
  echo ""
  echo "2. 编辑配置文件，填入你的机器人信息"
  echo ""
  echo "3. 在 opencode.config.js 中引入插件："
  echo "   import { NotifyPlugin } from './.opencode/plugins/opencode-plugin-notify/plugins/notify.js';"
fi
